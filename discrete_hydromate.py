import shutil
import numpy as np
import scipy.io.wavfile as wav
import matlab.engine
from bellhop_to_wav_2 import from_arr_to_wav
from joinaudio import join_audio_files

number_steps = 300

eng = matlab.engine.start_matlab() 
eng.cd('C:/Users/Nicola/Desktop/TESI/HYDRO2/hm_code', nargout=0)

# RX2 coordinates (FIXED)
Lat2 = -32.7572
Lon2 = 152.7698
prof = 200.0 # Remember the decimal (for MATLAB)

# Initial vessel coordinates
Lat_TX_init = -32.7562982878634
Lon_TX_init = 152.7698

# Final vessel coordiantes
Lat_TX_end = -32.7571999954564
Lon_TX_end = 152.770869496101

Lat_TXs = np.zeros(number_steps)
Lon_TXs = np.zeros(number_steps)
for i in range(number_steps):
    Lat_TXs[i] = Lat_TX_init + i*(Lat_TX_end - Lat_TX_init) / number_steps
    Lon_TXs[i] = Lon_TX_init + i*(Lon_TX_end - Lon_TX_init) / number_steps


for i,(Lat_TX,Lon_TX) in enumerate(zip(Lat_TXs,Lon_TXs)):
    eng.NB_prova1Func(Lat_TX,Lon_TX,Lat2,Lon2,prof) 

    # Copy the files from the MATLAB folder to the python folder
    for j in range(3):
        shutil.copyfile(f'C:/Users/Nicola/Desktop/TESI/HYDRO2/hm_code/test_5_{j+1}/test_5.arr',
                        f'C:/Users/Nicola/Desktop/TESI/Prove/TMP/{i}_RX{j+1}.arr')

    # By convolution, obtain a .wav file for each of the arrival files
    from_arr_to_wav(rx1=f'TMP/{i}_RX1.arr',
                    rx2=f'TMP/{i}_RX2.arr',
                    rx3=f'TMP/{i}_RX3.arr',
                    source = 'barca.wav',
                    out1 = f'TMP/{i}_RX1.wav',
                    out2 = f'TMP/{i}_RX2.wav',
                    out3 = f'TMP/{i}_RX3.wav',
                    n_arrivals=1000)
    
    # Clipping the tracks maintaining only the central portion
    durata = 1 # Seconds
    for j in range(3):
        fs, track = wav.read(f'TMP/{i}_RX{j+1}.wav')
        center = len(track) // 2
        track_clipped = track[center - int(fs/2*(durata)) : center + int(fs/2*(durata))]
        wav.write(f'TMP/{i}_RX{j+1}.wav',fs,track_clipped.astype(np.int16))
        
    if i == 0:
        for j in range(3):
            shutil.copyfile(f'TMP/{i}_RX{j+1}.wav',f'RX{j+1}.wav')
        
    if i > 0:
        for j in range(3):
            join_audio_files(f'RX{j+1}.wav',f'TMP/{i}_RX{j+1}.wav',f'RX{j+1}.wav')


eng.quit()